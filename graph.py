...
class Graph:
    def __init__(self, filename):
        ...
        self.pheromone_default_value = None

        random.seed()
        self._create_from_file(filename)

    def get_node_list(self):
        return [key for key in self._container if key is not None]
    ...
    def get_weight(self, node_from, node_to):
        if self._container[node_from] is not None:
    ...
    def get_denominator(self, pheromone_influence):
        ...
        for node_from in self._pheromones:
            result += sum([self._pheromones[node_from][node_to]**pheromone_influence *
                           self._container[node_from][node_to]**(1-pheromone_influence)
